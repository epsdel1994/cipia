 ...
 * @return turn's color.
 */
int board_set_c(Board *board, const char *string, unsigned long long *mask)
{
	int i;
	const char *s = string;

	board->player = board->opponent = 0ull;
    *mask = ~0ull;
	for (i = A1; i <= H8; ++i) {
		if (*s == '\0') break;
		switch (tolower(*s)) {
...
int get_mobility_c(const unsigned long long P, const unsigned long long O, const unsigned long long mask)
{
    return bit_count(get_moves(P,O) & mask);
}
 ...
 */
bool can_move_c(const unsigned long long P, const unsigned long long O, unsigned long long mask)
{
    return get_mobility_c(P, O, mask);
}
 ...
 */
char* board_to_string_c(const Board *board, const int player, char *s, const unsigned long long mask)
{
	int square, x;
	const char *color = "XO-=";
