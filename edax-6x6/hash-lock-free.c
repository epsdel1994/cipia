 ...
 */
static void data_update_c(HashData *data, const int cost, const int alpha, const int beta, const int score, const int move)
{
    if(data->upper > beta){ data->upper = (signed char)beta; }
    if(data->lower < alpha){ data->lower = (signed char)alpha; }
}
