...
static void position_symetry(Position *position, int s)
{
    Board board[1];
    *board = *position->board;
    board_symetry(board, s, position->board);
    for(int i=0; i<position->n_link; i++){
        position->link[i].move = symetry(position->link[i].move, s);
    }
    position->leaf.move = symetry(position->leaf.move, s);
}
...
//		if (r != 2 || header_edax != EDAX || header_book != BOOK) {
		if (r != 2 || header_edax != FCPA || header_book != BOOK) {
		...
		r += fread(&book->mask, sizeof book->mask, 1, f);
		r += fread(&book->P, sizeof book->P, 1, f);
		r += fread(&book->O, sizeof book->O, 1, f);
		if (r != 6) {
...
{
	unsigned int header_edax = FCPA, header_book = BOOK;
	...
	r += fwrite(&book->mask, sizeof book->mask, 1, f);
	r += fwrite(&book->P, sizeof book->P, 1, f);
	r += fwrite(&book->O, sizeof book->O, 1, f);
