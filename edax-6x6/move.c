 ...
 * @return move number.
 */
int movelist_get_moves_c(MoveList *movelist, const Board *board, const unsigned long long mask)
{
	Move *previous = movelist->move;
	Move *move = movelist->move + 1;
	unsigned long long moves = get_moves(board->player, board->opponent) & mask;
	int x;
